...
public class ServiceWatcher extends AbstractResourceWatcher<Service> {
	...
	protected String getUrl(String serviceName) {
		...
		Service service = services.stream().filter(svc -> svc.getSpec().getPorts().stream().filter(port -> port.getPort() == 8080).findFirst().isPresent()).findFirst().get();
		if(service == null) return null;
		String serviceURL = "";
		int port = 8080;
		try {
			port = service.getSpec().getPorts().stream().filter(servicePort -> servicePort.getPort()== 8080).findFirst().get().getPort();
		} catch (Exception e) {
